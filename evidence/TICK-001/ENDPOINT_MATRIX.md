...
| Reschedule appointment (FR-13) | None found | — | Exact route checks found no `PUT` or `PATCH /api/patient/:pid/appointment`, nor any mutating FHIR `Appointment` route. The Standard API exposes only appointment `GET`, `POST`, and `DELETE` routes ([source](https://github.com/openemr/openemr/blob/v8_3_0/apis/routes/_rest_routes_standard.inc.php#L397-L432)). | **Implementation-blocking API gap, confirmed at the service layer too (TICK-020, 2026-08-20).** No route AND no callable service method: `src/Services/AppointmentService.php` has no update method for an existing appointment's date/time/duration -- only ~150 lines of inline SQL in the legacy `interface/main/calendar/add_edit_event.php` page, tangled with recurrence/multi-provider branching, not a reusable business-logic entry point the way `insert()`/`updateAppointmentStatus()` are (see the resolved row below). The advertised OAuth scope cannot create a missing route, and there is no trusted internal method to delegate to. Do not emulate reschedule with delete/create, and do not write `pc_event` directly. |
| Cancel by status, retain history (FR-14) | None found in the Standard/FHIR route inventory | — | The only appointment mutation route is `DELETE /api/patient/{pid}/appointment/{eid}` ([source](https://github.com/openemr/openemr/blob/v8_3_0/apis/routes/_rest_routes_standard.inc.php#L422-L426)); its controller calls `deleteAppointmentRecord`, which conflicts with FR-14. | **Resolvable — not yet implemented (TICK-031, `status: todo`).** No route exists, but `AppointmentService::updateAppointmentStatus($eid, $status, $user)` (`src/Services/AppointmentService.php:591`) is real, callable OpenEMR business logic -- the same method the classic staff calendar UI's own cancel action calls, not a thin SQL wrapper -- and `list_options` (`apptstat`) has real cancelled-with-history status codes (`x`, `%`) distinct from `DELETE`. Reachable the same way TICK-017 added a route: `OpenEMR\Events\RestApiExtend\RestApiCreateEvent`, no core file touched. `updateAppointmentStatus` has no patient-binding check of its own (assumes a trusted caller, same as `insert()`); the new route adds the ownership check itself before calling it, the same pattern `AppointmentRestController::delete()` already uses. |
...
proceed. Any row still marked as a gap must not be implemented until a future
release, upstream-supported API extension, or approved product-scope change resolves
it. TICK-017's gap rows are resolved this way (an upstream-supported API extension —
OpenEMR's own module/event system, not a workaround). The cancel-by-status row TICK-020
split off as TICK-031 has the same resolution path available but is not yet built
(`status: todo`) — do not treat it as resolved until TICK-031 actually ships.
Reschedule (TICK-020, narrowed) has no such path and stays a gap: no service-layer
method exists to extend into, only inline SQL in a legacy calendar page. Direct MariaDB access, direct `pc_event` updates, native
form-table writes, a parallel scheduler, and delete/recreate emulation remain
