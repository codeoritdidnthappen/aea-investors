...
it. TICK-017's gap rows, and the cancel-by-status row TICK-020 split off as TICK-031,
are resolved this way (an upstream-supported API extension — OpenEMR's own
module/event system, not a workaround). Reschedule (TICK-020, narrowed) has no such
path and stays a gap: no service-layer method exists to extend into, only inline SQL
in a legacy calendar page. Direct MariaDB access, direct `pc_event` updates, native
